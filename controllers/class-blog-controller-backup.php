...
class Blog_Controller {
	...
	public function generate_from_approved_idea( $idea_id ) {
		...
		try {
			...
			$this->log_info( 'generate_from_approved_idea_not_approved', 'Idea not approved!', [
				'idea_id' => $idea_id,
			...
			return new \WP_Error( 'idea_not_approved', sprintf( 
				__( 'Idea must be approved to generate blog post. Current status: %s', 'ai-blog-generator' ), 
				$idea['status'] 
			) );
		}
	...
	/**
	 * AJAX handler to write debug messages to the debug-transaction.log file for troubleshooting
	 */
	public function ajax_write_debug() {
		...
		if ( empty( $message ) ) {
			wp_send_json_error( [ 'message' => __( 'Invalid message.', 'ai-blog-generator' ) ] );
		}
		...
		wp_send_json_success( [
			'message' => __( 'Debug message logged successfully.', 'ai-blog-generator' ),
		] );
	}
} 
